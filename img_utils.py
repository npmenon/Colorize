...
def concat_img_color(images, proportion, data_format):
    height = width = img = None
    if data_format == 'NHWC':
        height, width = images.shape[1], images.shape[2]
        img = np.zeros((height * proportion, width * proportion, 3))
    elif data_format == 'NCHW':
        height, width = images.shape[2], images.shape[3]
        img = np.zeros((3, height * proportion, width * proportion))
...
def concat_img(images, proportion, data_format):
    ...
    if data_format == 'NHWC':
        ...
        height, width = images.shape[2], images.shape[3]
        img = np.zeros((1, height * proportion, width * proportion))
...
def store_image(name, img, sess, data_format='NHWC'):
    print "saving img " + name
    if data_format == 'NCHW':
        img = sess.run(tf.transpose(img, [1, 2, 0]))
...
def sample_image_processing(data, batch_size, sess, data_format):
    ...
    store_image("output/base.png", concat_img_color(base_normalized, batch_size_sqrt, data_format), sess, data_format)
    store_image("output/base_line.jpg", concat_img(base_edge, batch_size_sqrt, data_format), sess, data_format)
    store_image("output/base_colors.jpg", concat_img_color(base_colors, batch_size_sqrt, data_format), sess, data_format)


def process_batch(batch_files, session, data_format='NHWC'):
    ...
    if data_format == 'NCHW':
        batch_normalized = session.run(tf.transpose(batch_normalized, [0, 3, 1, 2]))
        batch_edge = session.run(tf.transpose(batch_edge, [0, 3, 1, 2]))
        batch_colors = session.run(tf.transpose(batch_colors, [0, 3, 1, 2]))
